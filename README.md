...
## Project Overview
This repository contains the shell scripts developed for the Open Source Audit capstone project. It demonstrates practical Linux command-line skills and automates the inspection of Python's system footprint, dependencies, and operational logs.

## Required Dependencies 
To successfully execute the scripts in this repository, the following environment is required:
* **Operating System:** An Arch Linux based distribution (or an equivalent VM).
* **Shell:** Bash.
...
* **Description:** This script verifies if a specific open-source tool (defaulting to `python`) is installed on the system using the `dpkg` package manager. If installed, it outputs the version and architecture. It utilizes a case statement to append a brief philosophical note about the software's impact. [cite: 248, 126]
* **How to Run:**
  1. Open your terminal.
  2. Make the script executable: `chmod +x script2.sh`
  3. Execute the script (it defaults to checking MySQL): `./script2.sh`
  4. *Optional:* Test it with another package by passing an argument: `./script2.sh git`

### Script 3: Disk and Permission Auditor (`script3.sh`)
* **Description:** This script automates a security and storage audit. It iterates through an array of critical system directories and MySQL-specific paths (like `/var/lib/python` and `/etc/python`), extracting and displaying the read/write/execute permissions, the owning user/group, and the total disk space consumed by each directory. [cite: 248, 146, 147]
* **How to Run:**
  1. Open your terminal.
...
* **How to Run:**
  ...
  3. Execute the script with root privileges, providing the path to the MySQL error log and the keyword you want to search for: `sudo ./script4.sh /home/kunj/error.log warning`
